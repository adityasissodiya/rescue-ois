...
Promotion remains manual and operator-driven. In addition, every promotion will require a per-incident **promotion token** that must be presented alongside the host-level invocation:
...
- The first version of `promote-responder.sh` must be updated to enforce this; tests must cover the rejection path.
